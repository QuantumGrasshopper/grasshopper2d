...
> `./grasshopper -N 10000 -initconf random -gridsize 200 -d 0.3 -randomseed 123 -hours 0.2 -inittemp 20.0 -fintemp 0.05 `

Not all options need to be always specified, but specifying `N`, `d`, `randomseed`, and `hours` is a good idea. Also `initconf` needs to be specified.
...
- `config_plot.py` plots a grasshopper spin configuration in 2d, such as `initconf.dat` or `bestconf.dat` etc. Inputs are the name of the configuration file and the size of the grid.
- `anneal_animation.py` generates an animation of the simulated annealing process from the file `config.dat`, which is output by the main code. The size of the grid must be manually entered.
